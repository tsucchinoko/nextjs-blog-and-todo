...
import { getAllTaskIds, getAllTaskData, getTaskData } from "../../lib/tasks";

const fetcher = (url) => fetch(url).then((res) => res.json());

export default function Post({ staticFilteredTasks, id }) {
...
export async function getStaticProps({ params }) {
  const { task: staticFilteredTasks } = await getTaskData(params.id);

  return {
    props: {
      id: staticFilteredTasks.id,
      staticFilteredTasks,
    },
    revalidate: 3,
  };
